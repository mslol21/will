...
async function runSeed() {
  console.log("🌱 Enviando categorias via REST API...");
  ...
  console.log("Response Categorias:", catText || "OK");

  console.log("🌱 Enviando produtos via REST API...");
  const prodRes = await fetch(`${SUPABASE_URL}/rest/v1/produtos`, {
    method: "POST",
    headers,
    body: JSON.stringify(PRODUCTS),
  });
  console.log("Produtos Status:", prodRes.status);
  const prodText = await prodRes.text();
  console.log("Response Produtos:", prodText || "OK");

  console.log("\n🎉 SEED DE CATEGORIAS E PRODUTOS COMPLETO NO SUPABASE!");
}
